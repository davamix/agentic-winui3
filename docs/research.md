...
9. **How does a catalog express presentation *role* — heading vs body — for a `Text` component?** A style/variant property, or distinct component types? Raised by [experiment 01](./experiments/experiment-01-static-render.md): the first fixture's "title" rendered correctly and looked wrong, because nothing in the message says it is a heading. This is Q2 (catalog granularity) with a concrete forcing case. **Sharpened by [experiment 02](./experiments/experiment-02-binding-and-actions.md):** the agent's *reply* now renders identically to the form's own labels too, so the gap costs comprehension, not just polish.
10. **Where does protocol-flag validation belong, and what happens on mismatch?** Experiment 01 reads and logs `catalogId` but never checks the renderer implements it; [experiment 02](./experiments/experiment-02-binding-and-actions.md) adds `sendDataModel`, likewise neither honoured nor rejected. Silently-partial protocol support is accumulating: the renderer quietly does something other than what the producer asked for, and fails late and confusingly. Does a renderer need an explicit "what I implement" declaration it can refuse against?
...
12. **Who tears down a binding when a component is replaced?** [Experiment 02](./experiments/experiment-02-binding-and-actions.md) subscribes each bound control to the data model and never unsubscribes — safe only while the tree is built once and never replaced. Diffing needs a subscription scope tied to subtree lifetime, or a re-render leaks handlers that fire into dead controls. This is a *prerequisite* for Q11, not a consequence of it.
13. **What happens when producer and user write the same path at once?** Two-way binding gives every bound path two authors. Experiment 02 is last-writer-wins with no notion of "the user is mid-edit", so an `updateDataModel` can silently discard typing. Distinct from Q11: nothing is rebuilt, the value simply loses a race. Does A2UI need edit-in-progress semantics, or is that the renderer's problem?
14. **Which component properties may be dynamic, and who declares it?** Experiment 02 chose in C# that `value` and `text` are bindable while `label` is literal. A producer cannot discover that choice, and binding a "literal" property fails silently rather than erroring. Should the catalog schema declare per-property dynamism — and if so, this is Q2's granularity question reaching properties, not just components.
